...
object ClinVarSplitters {
  ...
  def getVariant(message: Msg): Option[Msg] =
    VariantTypes.foldLeft(Option.empty[Msg]) { (acc, subtype) =>
      acc.orElse {
  ...
  def unrollVariants(
    ...
    getVariant(variantWrapper).toIterable.flatMap {
      ...
      // Link upwards.
      variant.obj.update(ParentsRef, Arr(parentIds.to[mutable.ArrayBuffer]))
      immediateParentId.foreach(variant.obj.update(ParentRef, _))
